...
SELECT first_name, TO_CHAR(hire_date, 'YYYY"년" MM"월" DD"일"') AS datedf
FROM employees;
...
-- NULL 제거 함수 NVL2(컬럼, null이 아닐 경우 값, null일 경우)
SELECT
    NVL2(null, '널 아님', '널임')
FROM dual;
...
SELECT
    first_name,
    NVL2(commission_pct, 'true', 'false') AS oyeah
FROM employees;
...
-- DECODE(컬럼 혹은 표현식, 항목1, 결과1, 항복2, 결과2 ... default)
SELECT
...
*/
SELECT
    manager_id AS 사원번호 , first_name || last_name AS 사원명, hire_date AS 입사일자,
    TRUNC((sysdate  - hire_date) / 365) AS 근속년수
FROM employees
WHERE TRUNC((sysdate  - hire_date) / 365, 0) >= 17
ORDER BY 근속년수 DESC;
...
SELECT 
    ...
    DECODE(department_id, 100, '사원', 120, '주임', 121, '대리', 122, '과장', '임원') AS 임원
FROM employees
