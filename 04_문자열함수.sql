...
*/
 ...
 SELECT  
    'abcDEF', lower('abcDEF'), upper('abcDEF') , initcap('dddDDD')
 FROM 
    dual;
...
SELECT 
    'abcdef' AS ex,
    SUBSTR('abcdef', 1, 4), -- 1번부터 4번까지만
    CONCAT('abc', 'def') -- concat은 두개만 받음
FROM dual;
...
-- replace로 바꾸고 그 안에서 또 바꾸고 싶으면 replace를 한번 더 불러서 처리
SELECT 
...
*/
SELECT
    LPAD(SUBSTR(phone_number,5,14),12,'02.')  
FROM employees;
-- 내가 쓴거
--------------------------------------------
SELECT
