...
FROM
    (
    SELECT ROWNUM AS rn, tbl.hire_date
    FROM
        (
        SELECT e.employee_id, e.first_name || e.last_name AS name, e.phone_number, hire_date, d.department_id, d.department_name 
        FROM employees e
        LEFT JOIN departments d
        ON e.department_id = d.department_id
        ORDER BY hire_date ASC
...
FROM departments d
HAVING d.department_id IS NOT NULL;                     -- 내가 풀다가 실패한거
*/
...
ON d.department_id = tbl.departmt_id;
/*
문제 16
