...
-- 시퀀스 생성
CREATE SEQUENCE dept3_seq -- 시퀀스 생성은 괄호 x
    START WITH 1 -- 시작값 (기본값은 증가할 때 최소값, 감소할 떄 최대값), (,) 안하고
    INCREMENT BY 1 -- 증가값 (양수면 증가, 음수면 감소, 기본값 1)
    MAXVALUE 10 -- 최대값 (기본값은 증가 1027, 감소일 때 -1)
    MINVALUE 1 -- 최소값 (기본값 증가일 떄 1, 감소일 때 -1028)
    NOCACHE -- 캐시메몰 사용 여부 (CACHE)   
    NOCYCLE; -- 순환 여부 (NICYCLE이 기본, 순환시키려면 CYCLE)
...
ALTER SEQUENCE dept3_seq INCREMENT BY -1; -- 긍가값 변경
