...
SELECT
    ...
    e.salary, e.job_id, e.department_id, department_name
FROM employees e
...
테이블 이름을 작성하셔서 소속을 표햔해 주는 것이 바람직합니다.
...
AND
    e.job_id = j.job_id -- 해석순서 3 4위에꺼까지 
AND
    d.location_id = loc.location_id -- 해석순서 2
AND
    loc.state_province = 'California'; -- 해석순서 

/*
1. loc 테이블의 province = 'California' 조건에 맞는 값을 대상으로
. location_id 값과 같은 값을 가지는 데이터를
    departments에서 찾아서 조인
. 위의 결과와 동일한 department_id를 가진
    emplyees 테이블의 데이터를 찾아서 조인
. 위의 결과와 jobs 테이블을 비교하여 조인하고 최종 결과를 출력.
*/
